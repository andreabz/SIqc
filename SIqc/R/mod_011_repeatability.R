...
mod_011_repeatability_server <- function(id, r_global) {
  moduleServer(id, function(input, output, session) {
    ...
    # update data results when samples are changed
    shiny::observeEvent(c(input$sample1, input$sample2), {
      r_local$sample_results <- sql_get_repeatability(conn, input$sample1, input$sample2, mytask = r_global$taskid)
    })
    ...
    observeEvent(c(input$dt_data_cell_edit, input$result, input$comment),
                 {
